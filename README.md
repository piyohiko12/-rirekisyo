...
用紙の文字は **11pt** です（資格等・校内外の諸活動・志望の動機・備考・住所など）。

資格欄には **10件まで** 印字します。用紙の資格欄の高さ（141.75pt）が決まっており、
**その行は左側の欄（氏名・生年月日・現住所）と共有している**ため、資格欄だけ行を細かく
分けると様式全体が崩れます。そのため「11ptで10件」が上限です。
**欄からはみ出して印字されることはありません**（11件目以降は印字しません）。

> 件数に応じて行の高さや文字サイズを自動で変えるには、マクロ（.xlsm）が必要です。
