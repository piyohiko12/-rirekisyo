...
- Excelは**結合セルに必要な高さを教えてくれない**ため、測定用のセルで代用しています。
  実際の印刷はそれより多くの高さを必要とすることがあるので、余裕を持たせています。
  折り返しで行数が決まる**文章の欄**（諸活動・志望の動機・備考）はずれが行数ぶん
  積み上がるので、余裕を大きくしています。

  | | 高さの割り増し |
  | --- | --- |
  | 文章の欄（諸活動・志望の動機・備考） | **1.20倍** |
  | 改行で行数が決まる欄（資格など） | **1.05倍** |

  割り増しは**行数に比例**するので、文章が短いうちは 11pt のままです。
  この数値は `履歴書マクロ.bas` の先頭（MASHI / MASHI2）で調整できます。

  **志望の動機の目安**（改行なしの場合）

  | 文字数 | 〜277 | 〜334 | 〜404 | 〜461 | 〜526 | 〜600 | 〜942 |
  | --- | --- | --- | --- | --- | --- | --- | --- |
  | 文字の大きさ | 11pt | 10pt | 9pt | 8.5pt | 8pt | 7.5pt | 6pt |
