...
Pythonのインストールは要りません。マクロも必須ではありません
（文字の大きさを自動でそろえたいときだけ、任意で使えます）。
...
| 連絡先 | 空欄なら **「同上」** |
| 資格（手入力6枠＋取込8件） | **取得年月順に並べ替え**、**資格マスタで正式名称に変換**、和暦で表示（**最大14件**） |
...
- 手入力した資格と合わせて **取得年月の古い順** に並べ、資格マスタの正式名称で印字します（**最大14件**）
...
| | そのまま使う（マクロなし） | 「マクロ」シートの手順を1回だけ行う |
| --- | --- | --- |
| 文字の大きさ | ずっと 11pt | 欄ごとに **11pt〜6pt** で自動調整 |
| 資格が多い生徒 | 欄に入る分だけ印字（あふれた分は出ません） | **全件そのまま印字**（14件でも入ります） |
| 諸活動・志望の動機・備考が長い | 欄に入る分だけ印字 | **全文が欄に収まります** |
| 準備 | 不要 | .xlsm で保存 → C列のVBAを貼り付け（5分） |

**マクロ版**（ブックの「マクロ」シートに手順とコードが入っています）

- `履歴書の文字を整える` を実行すると、**資格等・校内外の諸活動・志望の動機・備考**の
  4つの欄について、欄の高さと幅をExcelから実寸で読み取り、
...
- E列のコード（任意）を `ThisWorkbook` に貼り付けると、**印刷・PDF出力の直前に自動実行**されます。
