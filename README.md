...
2. **「入力」シートに名列順で入力する**（1行＝1生徒・黄色いセル）
   ...
   - 生年月日は `2008/5/12` のように日付で入力 → **元号・満○歳は自動**
   - 連絡先を空欄にすると **自動で「同上」**
3. **「01」〜「40」のシートがそのまま履歴書**（数字＝入力シートのNo）
4. **PDFにする**
   - 1人分 … その生徒のシートを開いて「ファイル → 名前を付けて保存 → PDF」
   - 全員分 … 「ファイル → エクスポート → PDF/XPS → オプション → ブック全体」
   - 一部の生徒だけ … シート見出しを Ctrl キーで選び、オプションで「選択したシート」
...
- シート名（01〜40）と、非表示のシート（計算・資格集約）は変えないでください。
