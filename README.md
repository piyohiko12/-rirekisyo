...
- 行数は**文字数からの見積もりではありません**。非表示の「文字計測」シートに
  欄とまったく同じ幅の作業セルを用意し、同じフォント・同じ大きさで文章を入れて、
  **Excel自身に折り返させて必要な高さを実測**します。
...
- 短い生徒は 11pt のままです（無意味に小さくなりません）。
  ...
  **志望の動機の目安**（句読点を含むふつうの文章・改行3〜4か所）

  | 文字数 | 〜350 | 〜390 | 〜540 | 〜600 | 〜640 | 〜920 | 〜990 |
  | --- | --- | --- | --- | --- | --- | --- | --- |
  | 文字の大きさ | **11pt** | 10〜10.5pt | 9.5pt | 8.5〜9pt | 8pt | 7pt | 6.5pt |

  改行の数でも変わります（改行が多いほど1段階早く小さくなります）。
  上の表はあくまで目安で、実際の大きさは毎回実測して決めています。
- 6ptにしても入りきらないほど長い文章（志望の動機なら1000字を超えるあたり）が
  あったときは、実行後に**その件数を知らせます**。文章を短くしてください。
- 「文字計測」シートを消してしまっても動きます。そのときだけ、較正済みの計算式
  （`履歴書マクロ.bas` の先頭 HABA / GYOU）で行数を見積もります。

> 自動調整を入れれば、**欄からはみ出して印字されることはありません**
> （上の「知らせます」が出たときを除きます）。
> 入れない場合は 11pt のままなので、欄に入る分だけが印字されます。

### 注意

- 写真は印刷した用紙に貼ってください。
- 生年月日の「昭和・平成」、職歴の「平成・令和」の丸は、印刷後に手で付けてください。
- 非表示のシート（計算・資格集約・郵便番号・文字計測）は変えないでください。
