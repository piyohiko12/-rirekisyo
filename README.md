...
「マクロ」シートの **E列**（8行だけ・`"` を含まないので貼り付けても壊れません）を
`ThisWorkbook` に貼り付けると、**印刷・PDF出力の直前に自動で実行**されるので、
手順4も要らなくなります。

#### 何をしているか

- **資格等・校内外の諸活動・志望の動機・備考** の4つの欄について、
  欄の高さと幅を Excel から **実寸（ポイント）で読み取り**、文章がちょうど収まる
  文字の大きさ（**最大11pt・最小6pt・0.5pt刻み**）を欄ごとに決めます。
