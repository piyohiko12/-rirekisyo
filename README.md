...
| 履歴書 | 印刷する用紙（公式様式そのまま・1人＝1ページ・40ページ／資格10件・11pt） |
| 履歴書（資格が多い人） | 同じ用紙で資格を13件まで印字（文字は8.5pt・11件以上の生徒用） |
...
| 連絡先 | 空欄なら **「同上」** |
| 資格（手入力6枠＋取込8件） | **取得年月順に並べ替え**、**資格マスタで正式名称に変換**、和暦で表示（**最大13件**） |
...
- 手入力した資格と合わせて **取得年月の古い順** に並べ、資格マスタの正式名称で印字します（**最大13件**）
...
資格欄の高さ（141.75pt）は用紙で決まっているため、**件数と文字の大きさは両立しません**。
そこで、印刷用のシートを2枚用意しています。どちらも同じ入力内容を使います。

| シート | 件数 | 文字の大きさ |
| --- | --- | --- |
| 履歴書 | 10件まで | **11pt**（読みやすい既定） |
| 履歴書（資格が多い人） | 13件まで | 8.5pt（行の高さも詰めます） |

「設定」シートの2つ目のボタン【▶ 資格が多い人はこちら】から切り替えられます。
**欄からはみ出して印字されることはありません**（入りきらない分は印字しません）。
校内外の諸活動・志望の動機・備考も11ptにしました。

> 件数に応じて自動で切り替えるにはマクロ（.xlsm）が必要なため、シートを選ぶ形にしています。
