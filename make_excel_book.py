...
GUIDE = "使い方"
ZIPCODES = "郵便番号"
MEASURE = "文字計測"      # マクロが文字の高さを測るための作業シート（非表示）
FORM_SHEET = "履歴書"
...
def macro_code() -> list[str]:
    ...
    行数は、実際の印刷結果に合わせて較正した式で数える
    （Excelは結合セルに必要な高さを教えてくれないため）。
    """
...
Private Const SHEET_MEASURE As String = "{MEASURE}"
...
Public Function 文字を整える実行() As Boolean
    ...
    For i = 1 To STUDENT_COUNT
        ...
        手動 = 手動サイズ(i, {roster_index("size.licenses")})
        If 手動 > 0 Then pt = 手動
...
MACRO_STEPS = [
    ...
    "",
    "■ 大きさの決め方について",
    "　　欄に入りきるかどうかは、Excel自身に高さを測らせて判断しています。",
    "　　（非表示の「" + MEASURE + "」シートを使います。触らないでください）",
    "　　もう少し余白がほしいときは、C列の上のほうにある MASHI（ふつうの欄）と",
    "　　MASHI2（志望の動機）を 1 から 1.05 などに変えてください。",
    "　　6ポイントでも入りきらないときは、実行後にその件数をお知らせします。",
    "",
    "■ そのほか確かめること",
...
def build_measure_sheet(wb) -> None:
    """マクロが文字の高さを測るためだけの作業シート（ふだんは非表示）。

    A1 に文章を入れて行の高さを自動調整させ、履歴書の欄に収まるかを
    Excel自身に判定させる。ここを消すとマクロは予備の計算に切り替わる。
    """
    ws = wb.create_sheet(MEASURE)
    ws["A3"] = "このシートはマクロの作業用です。触らないでください（消しても動きます）。"
...
def build(official: Path, out: Path, students: list[dict] | None = None,
    ...
    build_measure_sheet(wb)
    build_guide(wb)

    order = [GUIDE, ROSTER, PASTE, SETTINGS, FIELDS, COURSES, MASTER,
             FORM_SHEET, MACRO_SHEET, GATHER, CALC, ZIPCODES, MEASURE]
